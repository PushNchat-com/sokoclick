...
import { validateWhatsAppNumber as validateWhatsAppNumberUtil } from "../utils/validationUtils";
...
const validateBasicInfo = (
  ...
  if (!formData.seller_whatsapp) {
    ...
    const validation = validateWhatsAppNumberUtil(formData.seller_whatsapp);
    if (!validation.isValid) {
      errors.seller_whatsapp = ctx.t({
        en: "WhatsApp number must be in Cameroon format (+237XXXXXXXXX)",
        fr: "Le numéro WhatsApp doit être au format camerounais (+237XXXXXXXXX)",
      });
    }
  }
...
export const validateWhatsAppNumber = (number: string): boolean => {
  const validation = validateWhatsAppNumberUtil(number);
