  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        const isTrustedAdmin = user.email
          ? trustedAdmins.includes(user.email.toLowerCase())
          : false;
        ...
        if (productId) {
          ...
          const hasAccess =
            isAdmin || (product && product.seller_id === user.id);
          ...
          if (!hasAccess) {
            console.log("Access denied: Cannot create product", {
              context,
              isAdmin,
            });
            navigate("/unauthorized");
