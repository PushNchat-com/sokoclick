...
export function useSlotBatchOperations(
  ...
  const showResultToast = useCallback(
    (result: BatchOperationResult, successMessage: string) => {
  ...
  const setMaintenanceStatus = useCallback(
    async (
      ...
      try {
        ...
        const result = await slotBatchOperations.setMaintenanceStatus(
          slotIds,
          operation,
        );
   ...
   */
  const clearMultipleSlots = useCallback(
    async (slotIds: number[]): Promise<BatchOperationResult> => {
      try {
        setState((prev) => ({
          ...prev,
          loading: true,
   ...
   */
  const reserveMultipleSlots = useCallback(
    async (
      slotIds: number[],
      endTime: Date,
      reservedBy: string,
      ...
      try {
        ...
        const result = await slotBatchOperations.reserveMultipleSlots(
          slotIds,
          endTime,
          reservedBy,
        );
   ...
   */
  const cancelMultipleReservations = useCallback(
    async (slotIds: number[]): Promise<BatchOperationResult> => {
      try {
        setState((prev) => ({
          ...prev,
        ...
        const result =
          await slotBatchOperations.cancelMultipleReservations(slotIds);
   ...
   */
  const getMultipleSlotsStatus = useCallback(
    async (slotIds: number[]): Promise<SlotOperationResponse<Slot[]>> => {
      try {
        setState((prev) => ({
          ...prev,
          loading: true,
        ...
        const result =
          await slotBatchOperations.getMultipleSlotsStatus(slotIds);
