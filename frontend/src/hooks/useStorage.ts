...
export function useStorage(
  ...
  const deleteFile = useCallback(
    async (bucket: StorageBucket, path: string): Promise<DeleteResult> => {
      setState((prev) => ({
        ...prev,
        isDeleting: true,
        error: null,
  ...
  // Clear slot images
  const clearSlotImages = useCallback(
    async (
      slotNumber: number,
    ): Promise<{ success: boolean; message: string }> => {
      setState((prev) => ({
      ...
      try {
        ...
        return {
          success: false,
          message:
            error instanceof Error ? error.message : "Failed to clear slot",
        };
      }
    },
    [handleError, showToasts, toast],
  ...
  const initializeSlotFolders = useCallback(async (): Promise<{
    success: boolean;
    message: string;
  }> => {
    setState((prev) => ({
    ...
    try {
      ...
      return {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to initialize slots",
      };
    }
  }, [handleError, showToasts, toast]);
