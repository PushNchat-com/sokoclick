...
export const useProductForm = ({
      ...
      if (isEditing && productId) {
        try {
          ...
          if (deliveryData && deliveryData.length > 0) {
            setDeliveryOptions(deliveryData.map(option => ({
              id: option.id,
  ...
  const handleDeliveryOptionsChange = (optionIndex: number, data: Partial<DeliveryOptionInternal>) => {
  ...
  const handleNextStep = () => {
    ...
    const validationErrors = validateStepData(currentStep, formData, safeImageFiles, deliveryOptions, isAdmin, t);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const safeImageFiles = Array.isArray(imageFiles) ? imageFiles : [];
      const step1Errors = validateStepData(1, formData, safeImageFiles, deliveryOptions, isAdmin, t);
      const step2Errors = validateStepData(2, formData, safeImageFiles, deliveryOptions, isAdmin, t);
      ...
      await Promise.all(deliveryOptions.map(async option => {
        // Skip empty options
      ...
      setSuccessMessage(t({
        en: isEditing ? 'Product updated successfully' : 'Product created successfully',
        fr: isEditing ? 'Produit mis à jour avec succès' : 'Produit créé avec succès'
      }));
