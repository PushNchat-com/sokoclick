...
export const useDebugSlots = () => {
  ...
  const fetchSlots = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `DEBUG - fetchSlots: Fetched ${slotsData?.length || 0} slots from database`,
      );
      ...
      const transformedSlots = await Promise.all(
        (slotsData || []).map(async (slot) => {
          if (slot.product_id) {
            console.log(
              `DEBUG - fetchSlots: Slot ${slot.id} has product_id ${slot.product_id}`,
            );
            try {
              ...
              if (productError) {
                console.error(
                  `DEBUG - fetchSlots: Error fetching product for slot ${slot.id}:`,
                  productError,
                );
              }
              ...
              if (productData) {
                console.log(
                  `DEBUG - fetchSlots: Found product for slot ${slot.id}: ${productData.name_en} (status: ${productData.status})`,
                );
                slot.product = productData;
              } else {
                console.warn(
                  `DEBUG - fetchSlots: No product data found for slot ${slot.id} with product_id ${slot.product_id}`,
                );
              }
            } catch (err) {
              console.warn(
                `DEBUG - fetchSlots: Exception fetching product for slot ${slot.id}:`,
                err,
              );
            }
          } else {
            console.log(
              `DEBUG - fetchSlots: Slot ${slot.id} has no product_id`,
            );
          }
          ...
          if (slot.is_maintenance) {
            ...
            slot.reservedUntil &&
            new Date(slot.reservedUntil) > new Date()
          ) {
          ...
          console.log(
            `DEBUG - fetchSlots: Slot ${slot.id} has status ${status}, is_active: ${slot.is_active}`,
          );
      ...
      // Print summary stats
      const slotsWithProducts = transformedSlots.filter(
        (slot) => slot.product_id,
      ).length;
      const slotsWithProductData = transformedSlots.filter(
        (slot) => slot.product,
      ).length;
      const activeSlots = transformedSlots.filter(
        (slot) => slot.is_active,
      ).length;
      const occupiedSlots = transformedSlots.filter(
