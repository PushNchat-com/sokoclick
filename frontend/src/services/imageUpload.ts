...
import { ImageFile, ImageValidationResult, IMAGE_CONSTRAINTS } from '../types/image';
...
class ImageUploadService {
  private async validateAndOptimizeImage(file: File): Promise<{ file: File; error?: string }> {
    // Validate image
    ...
    try {
      ...
      if (file.size > IMAGE_CONSTRAINTS.maxSize / 2) { // Only optimize if larger than 2.5MB
        const options = {
          maxSizeMB: IMAGE_CONSTRAINTS.maxSize / (1024 * 1024),
          maxWidthOrHeight: Math.max(IMAGE_CONSTRAINTS.maxDimensions.width, IMAGE_CONSTRAINTS.maxDimensions.height),
          useWebWorker: true
        };
  ...
  async uploadImage(
    ...
    try {
      ...
      const { file: optimizedFile, error: optimizationError } = await this.validateAndOptimizeImage(file);
      ...
      const { data, error } = await this.uploadWithRetry(optimizedFile, filePath);
  ...
  async uploadMultipleImages(
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!result.success) {
        failedUploads.push({ file: files[i], error: result.error || 'Unknown error' });
      }
    }
