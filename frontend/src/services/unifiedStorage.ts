...
class UnifiedStorageService {
  ...
  validateFile(file: File, options?: {
    ...
    const allowedTypes = options?.allowedTypes || IMAGE_CONSTRAINTS.allowedTypes;
  ...
  generatePath(file: File, options: UploadOptions = {}): string {
    ...
    if (options.slotNumber && options.slotNumber > 0 && options.slotNumber <= 25) {
   ...
   */
  async initializeSlotFolders(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      return { success: true, message: 'Slot folders initialized successfully' };
    } catch (error) {
      console.error('Failed to initialize slot folders:', error);
      return { 
        success: false, 
        message: error instanceof Error ? error.message : 'Unknown error initializing slot folders' 
      };
    }
  }
   ...
   */
  async clearSlotImages(slotNumber: number): Promise<{ success: boolean; message: string }> {
    try {
      if (slotNumber < 1 || slotNumber > 25) {
      ...
      return { success: true, message: `Cleared ${filesToDelete.length} files from slot ${slotNumber}` };
    } catch (error) {
      console.error(`Failed to clear slot ${slotNumber} images:`, error);
      return { 
        success: false, 
        message: error instanceof Error ? error.message : `Unknown error clearing slot ${slotNumber}` 
      };
    }
  }
