...
export interface SlotService {
  ...
  updateSlot(
    id: string,
    slot: Partial<SlotPayload>,
  ): Promise<ServiceResponse<Slot>>;
  ...
  batchRemoveProducts(slotIds: number[]): Promise<ServiceResponse<void>>;
}
...
class SlotServiceImpl extends BaseServiceImpl implements SlotService {
  ...
  async getSlots(filter?: SlotFilter): Promise<ServiceResponse<Slot[]>> {
    try {
      ...
      if (await this.isOnline()) {
        ...
        const { data, error } = await query.order("slotNumber", {
          ascending: true,
        });
        ...
        if (filter) {
          if (filter.status) {
            filteredSlots = filteredSlots.filter(
              (slot) => slot.status === filter.status,
            );
          }

          if (filter.productId) {
            filteredSlots = filteredSlots.filter(
              (slot) => slot.productId === filter.productId,
            );
          }
    ...
    try {
      if (await this.isOnline()) {
        ...
        if (error) {
          ...
          return createErrorResponse(
            "DATABASE_ERROR",
            `Failed to fetch slot with ID ${id}`,
          );
        }
      ...
      return createErrorResponse(
        "OFFLINE_ERROR",
        `Slot with ID ${id} not found in cache`,
      );
    } catch (error) {
  ...
  async createSlot(slot: SlotPayload): Promise<ServiceResponse<Slot>> {
    if (!(await this.isOnline())) {
      return createErrorResponse(
        "OFFLINE_ERROR",
        "Cannot create slots while offline",
      );
    }
    ...
    try {
      ...
      // Update cache
      const cachedSlots =
        (await this.getCachedData<Slot[]>(this.tableName)) || [];
  ...
  async updateSlot(
    id: string,
    slotUpdate: Partial<SlotPayload>,
  ): Promise<ServiceResponse<Slot>> {
    if (!(await this.isOnline())) {
      return createErrorResponse(
        "OFFLINE_ERROR",
        "Cannot update slots while offline",
      );
    }
    ...
    try {
      ...
      if (error) {
        ...
        return createErrorResponse(
          "DATABASE_ERROR",
          `Failed to update slot with ID ${id}`,
        );
      }
      ...
      const cachedSlots =
        (await this.getCachedData<Slot[]>(this.tableName)) || [];
      const updatedCachedSlots = cachedSlots.map((s) =>
        s.id === id ? (data as Slot) : s,
      );
  ...
  async deleteSlot(id: string): Promise<ServiceResponse<void>> {
    if (!(await this.isOnline())) {
      return createErrorResponse(
        "OFFLINE_ERROR",
        "Cannot delete slots while offline",
      );
    }
    ...
    try {
      ...
      if (error) {
        ...
        return createErrorResponse(
          "DATABASE_ERROR",
          `Failed to delete slot with ID ${id}`,
        );
      }
   ...
   * @param maintenance Whether to enable or disable maintenance mode
   * @returns Success/failure result
   */
  async batchSetMaintenance(
    slotIds: number[],
    maintenance: boolean,
  ): Promise<ServiceResponse<void>> {
    try {
      if (!this.isOnline) {
        return createErrorResponse(
          "OFFLINE_ERROR",
          "Cannot perform batch operations while offline",
        );
      }

      const { error } = await supabase.rpc("batch_set_slot_maintenance", {
        slot_ids: slotIds,
        maintenance_status: maintenance,
      });

      if (error) {
        console.error("Batch maintenance operation failed:", error);
        return createErrorResponse(
          "DATABASE_ERROR",
          `Failed to update maintenance status for slots: ${error.message}`,
        );
      }
      ...
      console.error(
        "Unexpected error during batch maintenance operation:",
        error,
      );
      return createErrorResponse(
        "UNKNOWN_ERROR",
  ...
  /**
   * Remove products from multiple slots at once
   * @param slotIds Array of slot IDs to update
   * @returns Success/failure result
   */
  async batchRemoveProducts(slotIds: number[]): Promise<ServiceResponse<void>> {
    try {
      if (!this.isOnline) {
        return createErrorResponse(
          "OFFLINE_ERROR",
      ...
      const { error } = await supabase.rpc("batch_remove_products_from_slots", {
        slot_ids: slotIds,
      });

      if (error) {
        console.error("Batch product removal failed:", error);
        return createErrorResponse(
          "DATABASE_ERROR",
          `Failed to remove products from slots: ${error.message}`,
        );
      }

      return createSuccessResponse(undefined);
    } catch (error) {
      console.error("Unexpected error during batch product removal:", error);
      return createErrorResponse(
        "UNKNOWN_ERROR",
        "An unexpected error occurred",
      );
