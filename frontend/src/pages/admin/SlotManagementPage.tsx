...
const SlotManagementPage: React.FC = () => {
  ...
  const text = {
    ...
    error: { en: 'Failed to load slots', fr: 'Impossible de charger les emplacements' },
    maintenanceSuccess: { en: 'Maintenance status updated', fr: 'Statut de maintenance mis à jour' },
    maintenanceError: { en: 'Failed to update maintenance status', fr: 'Impossible de mettre à jour le statut de maintenance' },
    reserveSuccess: { en: 'Slot reserved successfully', fr: 'Emplacement réservé avec succès' },
    reserveError: { en: 'Failed to reserve slot', fr: 'Impossible de réserver l\'emplacement' },
    cancelSuccess: { en: 'Reservation cancelled', fr: 'Réservation annulée' },
    cancelError: { en: 'Failed to cancel reservation', fr: 'Impossible d\'annuler la réservation' }
  };
  ...
  const filteredSlots = slots.filter(slot => {
    ...
    if (searchTerm.trim() !== '') {
      ...
      return (
        ...
        (slot.description && slot.description.toLowerCase().includes(searchLower))
      );
    }
  ...
  const handleToggleMaintenance = async (slotId: string) => {
    try {
      ...
      const newStatus = slotToUpdate.status === SlotStatus.MAINTENANCE 
