...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const productDescription = language === 'en' ? product.description_en : product.description_fr;
  const categoryName = product.category ? (language === 'en' ? product.category.name_en : product.category.name_fr) : '';
  const formattedPrice = new Intl.NumberFormat(language === 'en' ? 'en-US' : 'fr-FR', {
    style: 'currency',
  ...
  return (
    <div className={`product-card border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow ${className}`}>
      {/* Product image */}
      ...
      <div className="p-4">
        ...
        {productDescription && (
          <p className="text-sm text-gray-700 mt-2 line-clamp-2">{productDescription}</p>
        )}
        ...
        {isAdmin && (
          <div className="mt-4 border-t pt-4">
            <div className="flex flex-wrap gap-2">
              ...
              >
                {product.auction_slot_id ? t(text.changeSlot) : t(text.assignSlot)}
              </button>
              ...
              >
                ...
                <option value={ProductStatus.APPROVED}>{t(text.approved)}</option>
                <option value={ProductStatus.REJECTED}>{t(text.rejected)}</option>
                <option value={ProductStatus.INACTIVE}>{t(text.inactive)}</option>
              </select>
            </div>
          </div>
        )}
            ...
            {product.seller?.whatsapp_number && (
              ...
              >
                <svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M12 1.5C6.477 1.5 2 5.977 2 11.5c0 1.8.472 3.485 1.302 4.949L2 21.5l5.184-1.36A9.96 9.96 0 0012 21.5c5.523 0 10-4.477 10-10S17.523 1.5 12 1.5zm0 18a8 8 0 01-4.099-1.125l-.293-.174-3.048.796.813-2.964-.183-.311A7.967 7.967 0 014 11.5a8 8 0 118 8z" fillRule="evenodd" clipRule="evenodd" />
                </svg>
