  ...
  return (
    <div className={`rounded-lg border border-gray-200 p-4 ${className}`}>
      ...
      {options.length > 0 ? (
        <ul className="space-y-3">
          {options.map((option, index) => (
            <li key={index} className="flex justify-between items-center p-3 rounded-md border border-gray-100 hover:bg-gray-50">
              <div>
                <p className="font-medium text-gray-900">{option.type}</p>
                <p className="text-sm text-gray-500">{option.estimatedDelivery}</p>
              </div>
      ...
      <p className="mt-4 text-xs text-gray-500">
        * Actual delivery times may vary based on shipping address and carrier conditions
      </p>
    </div>
  );
};
