...
const SlotProductPanel: React.FC<SlotProductPanelProps> = ({ slotId }) => {
  ...
  // Text content
  const text = {
    title: { en: 'Slot Product Management', fr: 'Gestion des produits de l\'emplacement' },
    ...
    error: { en: 'Error loading data', fr: 'Erreur lors du chargement des données' },
  ...
  const handleAssignProduct = async (days = 7) => {
    ...
    try {
      const result = await slotService.assignProductToSlot(slotId, selectedProductId, days);
  ...
  const getStatusText = (slotStatus?: string) => {
    ...
    switch (slotStatus.toUpperCase()) {
      case 'OCCUPIED': return t(text.statusOccupied);
      case 'RESERVED': return t(text.statusReserved);
      case 'MAINTENANCE': return t(text.statusMaintenance);
      default: return t(text.statusAvailable);
    }
  };
  ...
  if (slotError) {
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <div className="text-red-500 mb-4">{t(text.error)}: {slotError}</div>
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="mb-6 border-b pb-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="font-medium">{t({ en: 'Status', fr: 'Statut' })}:</div>
          <div>{getStatusText(slot?.status)}</div>
          
          {slot?.end_time && (
            <>
              <div className="font-medium">{t({ en: 'Ends', fr: 'Termine' })}:</div>
        ...
        <div className="mb-6 border-b pb-4">
          ...
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600">{slot.product.price.toLocaleString()} {slot.product.currency}</p>
              {slot.product.category && (
                <p className="text-xs text-gray-500">{getCategoryName(slot.product)}</p>
              )}
            </div>
          </div>
                ...
                {products.map(product => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-medium text-sm">{getProductName(product)}</div>
