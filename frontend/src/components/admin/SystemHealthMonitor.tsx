...
const SystemHealthMonitor: React.FC<SystemHealthMonitorProps> = ({
  ...
  const text = {
    ...
    noIssues: { en: "No issues detected", fr: "Aucun problème détécté" },
  ...
  const fetchHealthData = useCallback(async () => {
    ...
    try {
      ...
      if (healthResponse.success && healthResponse.data) {
        ...
        const alertCount = healthResponse.data.criticalErrorCount;
        if (onErrorsDetected && alertCount > 0) {
          onErrorsDetected(alertCount);
        }

        // Also call the new onAlertCountChange prop
  ...
  }, [onErrorsDetected, onAlertCountChange]);
  ...
  const renderStatusBadge = (
    status: "healthy" | "degraded" | "unhealthy" | "up" | "down" | "degraded",
  ) => {
    let colorClass = "";
        ...
        <>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-medium text-gray-500">
                  {t(text.criticalErrors)}
                </span>
                <div className="mt-1 flex items-center">
                  <span
                    className={`text-lg font-medium ${
                      healthStatus.criticalErrorCount > 0
                        ? "text-red-600"
                        : "text-gray-900"
                    }`}
                  >
                  ...
                  {healthStatus.services.map((service) => (
                    <tr key={service.name}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                        {service.latency
                          ? `${service.latency.toFixed(0)} ${t(text.ms)}`
                          : "-"}
                      </td>
                    </tr>
                  ))}
                        ...
                        <tr key={error.id}>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex space-x-2 justify-end">
                              ...
                              <button
                                onClick={() =>
                                  handleAcknowledgeError(error.id!)
                                }
