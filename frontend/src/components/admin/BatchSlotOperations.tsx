...
const BatchSlotOperations: React.FC = () => {
  ...
  const slotsByStatus = {
    ...
    [SlotStatus.OCCUPIED]:
      slots?.filter((slot) => slot.status === SlotStatus.OCCUPIED) || [],
    [SlotStatus.RESERVED]:
      slots?.filter((slot) => slot.status === SlotStatus.RESERVED) || [],
    [SlotStatus.MAINTENANCE]:
      slots?.filter((slot) => slot.status === SlotStatus.MAINTENANCE) || [],
  };
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="mb-6">
        ...
        <div className="text-sm mb-2">
          ...
          {selectedSlots.length} / {slots?.length || 0}{" "}
          {t({
            en: "slots",
            fr: "slots",
          })}
                  ...
                  <div className="flex items-center">
                    <Checkbox
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        handleSlotSelection(slotId, e.target.checked)
                      }
                      id={`slot-${slotId}`}
                      className="mr-2"
                    />
          ...
          <div className="mb-4">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2.5">
              <div
                ...
                style={{
                  width: `${(operationProgress / operationTotal) * 100}%`,
                }}
              ></div>
            </div>
          </div>
