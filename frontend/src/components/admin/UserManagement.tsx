...
const UserManagement: React.FC = () => {
  ...
  const { users, loading, error, total, refresh } = useUsers(selectedRole, searchQuery);
  ...
  const handleFixRecursion = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert(language === 'en' ? 'Recursion issues fixed!' : 'Problèmes de récursion résolus!');
        refresh();
      } else {
        alert(language === 'en' 
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">{text.title[language]}</h1>
        ...
        <div className="p-4 border-b">
          <div className="flex flex-col md:flex-row gap-4 justify-between">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <option value={UserRole.CUSTOMER}>{text.customer[language]}</option>
          ...
          <div className="mt-4 flex flex-wrap justify-between items-center">
            <div className="mb-2 md:mb-0">
              <span className="text-sm font-semibold">{text.totalUsers[language]}:</span> {total}
            </div>
                ...
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                      <button
                        onClick={() => {
                          userService.updateVerificationStatus(user.id, !user.isVerified)
                            .then(result => {
                      ...
                      >
                        {user.isVerified ? (
                          language === 'en' ? 'Unverify' : 'Déverifier'
                        ) : (
