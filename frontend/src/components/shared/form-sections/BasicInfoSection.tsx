...
const BasicInfoSection: React.FC<BasicInfoSectionProps> = ({
  ...
  const text = {
    ...
    productNameEn: { en: 'Product Name (English)', fr: 'Nom du Produit (Anglais)' },
    productNameFr: { en: 'Product Name (French)', fr: 'Nom du Produit (Français)' },
    ...
    whatsAppNumberHelp: { en: 'Enter in international format (e.g., +237XXXXXXXXX)', fr: 'Entrer au format international (ex: +237XXXXXXXXX)' },
    category: { en: 'Category', fr: 'Catégorie' },
    selectCategory: { en: 'Select a category', fr: 'Sélectionnez une catégorie' },
  ...
  const handleBlur = (fieldName: string) => {
    ...
    if (fieldName === 'seller_whatsapp') {
      validateWhatsAppNumber(formData.seller_whatsapp);
    }
  };
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg border border-gray-200 shadow-sm">
      <h3 className="text-xl font-medium text-gray-900 mb-6">{t(text.basicInfo)}</h3>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div>
          ...
          <input
            ...
            placeholder={t({ en: 'Enter product name', fr: 'Entrez le nom du produit' })}
            aria-required="true"
            aria-invalid={!!errors.name_en}
            aria-describedby={errors.name_en ? "name_en_error" : undefined}
          />
          {errors.name_en && <p id="name_en_error" className="mt-1 text-sm text-red-600">{errors.name_en}</p>}
        ...
        {/* Product Name (French) */}
        <div>
          <label htmlFor="name_fr" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <input
            ...
            placeholder={t({ en: 'Enter product name in French', fr: 'Entrez le nom du produit en français' })}
            aria-required="true"
            aria-invalid={!!errors.name_fr}
            aria-describedby={errors.name_fr ? "name_fr_error" : undefined}
          />
          {errors.name_fr && <p id="name_fr_error" className="mt-1 text-sm text-red-600">{errors.name_fr}</p>}
        ...
        <div className="sm:col-span-2">
          ...
          <textarea
            ...
            placeholder={t({ en: 'Describe your product', fr: 'Décrivez votre produit' })}
            aria-required="true"
            aria-invalid={!!errors.description_en}
            aria-describedby={errors.description_en ? "description_en_error" : undefined}
          />
          {errors.description_en && <p id="description_en_error" className="mt-1 text-sm text-red-600">{errors.description_en}</p>}
          {!errors.description_en && focused.description_en && !formData.description_en && (
            <p className="mt-1 text-sm text-orange-500">{t(text.required)}</p>
          )}
        </div>
        ...
        {/* Description (French) */}
        <div className="sm:col-span-2">
          <label htmlFor="description_fr" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <textarea
            ...
            placeholder={t({ en: 'Describe your product in French', fr: 'Décrivez votre produit en français' })}
            aria-required="true"
            aria-invalid={!!errors.description_fr}
            aria-describedby={errors.description_fr ? "description_fr_error" : undefined}
          />
          {errors.description_fr && <p id="description_fr_error" className="mt-1 text-sm text-red-600">{errors.description_fr}</p>}
          {!errors.description_fr && focused.description_fr && !formData.description_fr && (
            <p className="mt-1 text-sm text-orange-500">{t(text.required)}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.price && <p id="price_error" className="mt-1 text-sm text-red-600">{errors.price}</p>}
        ...
        {/* WhatsApp Number */}
        <div>
          <label htmlFor="seller_whatsapp" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div className="relative">
            <input
              ...
              maxLength={15}
              pattern="^\+[0-9]{1,15}$"
              className={`bg-white border pl-10 ${errors.seller_whatsapp ? 'border-red-500' : focused.seller_whatsapp && !validateWhatsAppNumber(formData.seller_whatsapp) ? 'border-orange-300' : 'border-gray-300'} text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5`}
              aria-required="true"
          ...
          <p id="whatsapp_help" className="mt-1 text-xs text-gray-500">{t(text.whatsAppNumberHelp)}</p>
          {errors.seller_whatsapp && <p id="seller_whatsapp_error" className="mt-1 text-sm text-red-600">{errors.seller_whatsapp}</p>}
          {!errors.seller_whatsapp && focused.seller_whatsapp && !validateWhatsAppNumber(formData.seller_whatsapp) && (
            <p className="mt-1 text-sm text-orange-500">{t({ en: 'Please enter a valid WhatsApp number with country code (+237...)', fr: 'Veuillez entrer un numéro WhatsApp valide avec indicatif pays (+237...)' })}</p>
          )}
        </div>
        ...
        <div>
          ...
          {errors.category && <p id="category_error" className="mt-1 text-sm text-red-600">{errors.category}</p>}
