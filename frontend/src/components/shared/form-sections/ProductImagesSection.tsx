...
const ProductImagesSection: React.FC<ProductImagesSectionProps> = ({
  ...
  // Handle drag events
  const handleDrag = useCallback((e: React.DragEvent<HTMLDivElement>, index: number, isDragActive: boolean) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    <div className="bg-white p-4 sm:p-6 rounded-lg border border-gray-200 shadow-sm">
      ...
      <div className="space-y-6">
        <div className="flex flex-col space-y-2 text-sm text-gray-500">
          <p>{t({ en: 'Maximum file size: 5MB', fr: 'Taille maximale: 5Mo' })}</p>
          <p>{t({ en: 'Supported formats: JPG, PNG, WebP', fr: 'Formats supportés: JPG, PNG, WebP' })}</p>
        </div>
          ...
          {images.map((image, index) => (
            <div
              ...
              className={`relative border-2 ${
                errors[`image_${index}`] ? 'border-red-300 bg-red-50' : 
                dragActive === index ? 'border-blue-400 bg-blue-50' : 
              ...
              {(image.url || image.preview) ? (
                    ...
                    <div className="absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center">
                      <div className="w-full max-w-md px-4">
                        <div className="mb-1 text-sm text-center font-medium text-blue-700">
                          {t({ en: 'Uploading...', fr: 'Téléchargement...' })} ({Math.round(image.progress)}%)
                        </div>
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-gray-600">
                    {index === 0 ? 
                      t({ en: 'Main Product Image', fr: 'Image principale du produit' }) :
                      t({ en: 'Additional Image', fr: 'Image supplémentaire' })
                    }
                  </p>
                  <p className="mt-1 text-xs text-gray-500">
                    {t({ en: 'Click or drag to upload', fr: 'Cliquez ou glissez pour télécharger' })}
                  </p>
                </div>
              )}
