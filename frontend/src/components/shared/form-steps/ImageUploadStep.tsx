...
const ImageUploadStep: React.FC<ImageUploadStepProps> = ({
  ...
  const text = {
    ...
    imageUploadHelp: { en: 'Upload up to 5 images (2MB max per image)', fr: 'Téléchargez jusqu\'à 5 images (2MB max par image)' },
    retryUpload: { en: 'Retry Upload', fr: 'Réessayer' },
    uploadingImage: { en: 'Uploading image...', fr: 'Téléchargement en cours...' },
    uploadError: { en: 'Upload failed. Try again.', fr: 'Échec du téléchargement. Essayez à nouveau.' },
  };

  // Handle image upload operation
  const performUpload = useCallback(async (imageFiles: ImageFile[], imageIndex: number) => {
    try {
      // Reset debug info
      setDebugInfo(null);
      ...
      const updatedImages = imageFiles.map((img, index) => 
        index === imageIndex
          ? { ...img, progress: 0, error: null }
          : img
      );
      onChange(updatedImages);

      // Log upload attempt
      console.log(`Attempting to upload image to bucket: ${DEFAULT_BUCKET}, productId: ${productId || 'none'}`);
      ...
      setDebugInfo(`Unexpected error: ${error instanceof Error ? error.message : 'Unknown error'}`);
