...
const TextLogoFallback: React.FC<{ text: string; className?: string }> = ({ text, className }) => {
...
const ResponsiveImage: React.FC<ResponsiveImageProps> = ({
  ...
  const defaultSizes = sizes || calculateSizes({
    '(max-width: 640px)': 100,
    '(max-width: 768px)': 50,
    '(max-width: 1024px)': 33,
    '(min-width: 1025px)': 25,
  ...
  const handleErrorWithFallback = (e: React.SyntheticEvent<HTMLImageElement>) => {
  ...
  const sizeClass = imageSize ? 
    imageSize === 'product' ? 'h-product-image' :
    imageSize === 'thumbnail' ? 'h-thumbnail' :
    `h-image-${imageSize}` : '';
